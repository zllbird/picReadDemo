 ...
 */
class MyLoggingIntercetor @JvmOverloads constructor(private val logger: Logger = Logger.DEFAULT) :
    Interceptor {
    ...
    fun setLevel(level: Level?): MyLoggingIntercetor {
...
//        mSchemeDao?.insert(SchemeItem(0,"time is $tookMs"))
        mSchemeDao?.insert(SchemeItem(0,"code:${response.code()}:timeuse:$tookMs(ms)", tookMs, response.code()))
